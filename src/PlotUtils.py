...
def plot_phase_aligned_averages_combined(segmented_steps, feature_keys, n_points=100):
    """
    Plot normalized average trajectories (with SEM) for multiple features
    on the same figure, colored by majority gait phase.
    ...
    for feature_key in feature_keys:
        ...
        for step in segmented_steps:
            ...
            # Interpolate phase labels (nearest neighbor)
            interp_indices = np.round(np.linspace(0, len(values) - 1, n_points)).astype(int)
            interp_phases = phase_labels[interp_indices]
        ...
        if not all_curves:
            print(f"Warning: No valid steps for feature '{feature_key}'")
            continue
        ...
        # Plot line segments for the feature, colored by phase
        for i in range(n_points - 1):
            phase = majority_phase[i]
            ...
            fig.add_trace(go.Scatter(
                ...
                name=f'{feature_key} ({phase})' if i == 0 else None,
        ...
        fig.add_trace(go.Scatter(
            ...
            line=dict(color='rgba(255,255,255,0)'),
            name=f'SEM ({feature_key})',
            legendgroup=feature_key,
            showlegend=False
        ))

    fig.update_layout(
        title="Phase-Aligned Average Trajectories (Multiple Features)",
